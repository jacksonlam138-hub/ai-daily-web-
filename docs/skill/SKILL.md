...
### 步骤4: 生成日报
网页端内容规则（摘要与角色解读分别保存）：
- 摘要以 50–80 字、1–2 句为目标，必要的来源归属与争议限定可保留至 100 字。只写核心事实，不堆背景、评价或建议，不用截断/省略号掩盖冗长内容。
- 每个相关角色通常 1–2 项，总计以 40–100 字为目标；每项为“1. 短标题：一句解释。”，编号项之间用换行符 `\n` 分隔，不用分号串接。
- 短标题 2–6 字；一项一个判断或低成本行动。省去开场总论、复述新闻、结尾口号，保留必要的前提与不确定性。
- 角色标题由页面渲染，例如【产品视角】，无需写进字段。没有直接相关性时省略该角色，不为覆盖人数或条数凑建议。
- 示例：`pm: '1. 数据边界：明确用户材料的训练用途与退出方式。\n2. 结果验证：为研究结论保留引用和复现步骤。'`
...
## 注意事项
- Markdown 推送摘要控制在50字以内；网页摘要按步骤4的规则编辑
- 每个分类最多5条内容
- 角色视角是差异化价值，但不相关时必须留空，不强行补充 PM 建议
